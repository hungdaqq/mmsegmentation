    ...
    for i in range(num_classes):
        if i != ignore_index:
            ...
            if class_weight is not None:
                tkbub_loss *= class_weight[i]
            total_loss += tkbub_loss
    return total_loss / num_classes


@weighted_loss
def binary_tkbub_loss(pred,
                    target,
                    valid_mask,
                    gamma=4/3,
                    smooth=1e-10):
...
class TKBUBv2Loss(nn.Module):
    def __init__(self,
                 ...
                 ignore_index=255,
                 gamma = 4/3,
                 loss_name='loss_tkbub'):
        super(TKBUBv2Loss, self).__init__()
        self.smooth = smooth
    ...
    def forward(self, pred, target, **kwargs):
        ...
        loss = self.loss_weight * tkbub_loss(
            pred,
            one_hot_target,
            valid_mask=valid_mask,
            gamma=self.gamma,
